...
CSV columns:
  frame_id, timestamp_s, track_id, class_id, class_name, conf,
  class_id2, class_name2, conf2,
  state, x1, y1, x2, y2, width, height, cx, cy

  conf         - classification confidence for class_id (top-1), 0-1
  class_id2    - second most likely class id (-1 if unavailable)
  class_name2  - name of class_id2 ("n/a" if unavailable)
  conf2        - confidence of class_id2 (blank if unavailable, e.g. SAHI mode)
"""
...
CSV_COLUMNS = [
    ...
    "class_id2",    # second most likely class id (-1 if unavailable)
    "class_name2",  # name of class_id2 ("n/a" if unavailable)
    "conf2",        # confidence of class_id2 (blank if unavailable)
    ...
    def write(self, frame_id: int, timestamp_s: float, tracks: list):
        for t in tracks:
            ...
            self._writer.writerow({
                ...
                "conf":        round(t.get("conf", float("nan")), 4),
                "class_id2":   t.get("class_id2", -1),
                "class_name2": class2_name(t.get("class_id2", -1)),
                "conf2":       "" if conf2 is None or conf2 != conf2  # NaN check
                               else round(conf2, 4),
                "state":       t["state"],
                "x1":          round(x1, 2),
                "y1":          round(y1, 2),
...
def draw_tracks(frame: np.ndarray, tracks: list,
    ...
    for t in tracks:
        ...
        conf      = t.get("conf", float("nan"))
        conf_str  = "" if conf != conf else f' {conf*100:.0f}%'
        label     = f'{CLASS_NAMES.get(t["class_id"], "obj")} #{t["id"]}{conf_str}'
        ...
        # Second most-likely class, drawn as a smaller line under the box
        conf2 = t.get("conf2", float("nan"))
        if conf2 == conf2:   # not NaN -> available
            label2 = f'2nd: {class2_name(t.get("class_id2", -1))} {conf2*100:.0f}%'
            font_scale2 = font_scale * 0.85
            (tw2, th2), _ = cv2.getTextSize(label2, cv2.FONT_HERSHEY_SIMPLEX, font_scale2, 1)
            lx2, ly2 = x1, y2 + th2 + 6
            cv2.rectangle(vis, (lx2, ly2 - th2 - 3), (lx2 + tw2 + 4, ly2 + 2), color, -1)
            cv2.putText(vis, label2, (lx2 + 2, ly2 - 1),
                        cv2.FONT_HERSHEY_SIMPLEX, font_scale2, (0, 0, 0), 1, cv2.LINE_AA)
